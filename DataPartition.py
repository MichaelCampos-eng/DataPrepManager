...
class DataPartition:
    ...
    def move_to_partition(self, part_path: str):
        ...
        if not os.path.exists(self.part_path):
            raise DirectoryError("Directory {} does not exist!".format(self.part_path))

        self.__move_to_partition(self.part_path)
    
    # OK
    def move_to_partition(self, index: int):
        if not os.path.exists(self.source_path):
            raise DirectoryError("Directory {} does not exist!".format(self.source_path))
        if not os.path.exists(self.partitions[index]):
            raise DirectoryError("Directory {} does not exist!".format(self.partitions[index]))
    ...
    # OK
    def partition_by_basename(self):
        for partition in self.partitions:

            basename = os.path.basename(partition)
            for file in os.listdir(self.source_path):
                if basename in file:
                    shutil.move(os.path.join(self.source_path, file), partition)
